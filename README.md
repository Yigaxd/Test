...
--
-- PLANILLA ORIGINAL (DESNORMALIZADA):
-- ORDEN_SERVICIO(ID_ORDEN, FECHA, RUT_CLIENTE, NOM_CLIENTE, FONO_CLIENTE,
--               PATENTE_AUTO, MARCA_AUTO, ID_REPUESTO, NOM_REPUESTO,
--               PRECIO_UNITARIO, CANTIDAD, MECANICO_RUT, MECANICO_NOM)
--
-- 1. PRIMERA FORMA NORMAL (1FN)
--
-- DEFINICIÓN: Una tabla está en 1FN si todos sus atributos contienen valores
-- atómicos (no divisibles), no hay grupos repetitivos y existe una clave primaria.
--
-- ANOMALÍAS DETECTADAS EN LA PLANILLA ORIGINAL:
--
-- (a) GRUPOS REPETITIVOS / FILAS DUPLICADAS:
...
--     Ejemplo:
--       Fila 1: 5001 | Carlos Tapia | DL-PP-40 | R01 | Filtro de Aceite | ...
--       Fila 2: 5001 | Carlos Tapia | DL-PP-40 | R02 | Aceite Sintético  | ...
--
-- (b) AUSENCIA DE CLAVE PRIMARIA SIMPLE:
...
--     tabla de detalles, eliminando la ambigüedad de identificación. Esto
--     satisface 1FN al garantizar que cada fila sea única e identificable.
--
-- 2. SEGUNDA FORMA NORMAL (2FN)
-- 
-- DEFINICIÓN: Una tabla está en 2FN si está en 1FN y todos los atributos
-- no clave dependen COMPLETAMENTE de la clave primaria (no existen dependencias
-- parciales, es decir, dependencias con solo una parte de la clave compuesta).
--
...
--     (el nombre y precio de un repuesto son propios del repuesto, no del
--     par orden+repuesto)
--
...
-- =============================================================================
-- DEFINICIÓN: Una tabla está en 3FN si está en 2FN y no existen dependencias
-- TRANSITIVAS, es decir, ningún atributo no clave depende de otro atributo
-- no clave (todo atributo no clave debe depender directamente de la PK).
--
-- ANOMALÍAS DETECTADAS (dependencias transitivas):
--
-- En la tabla ORDEN (tras aplicar 2FN) aún persisten:
--
--   → ID_ORDEN → RUT_CLIENTE → NOM_CLIENTE, FONO_CLIENTE
--     (NOM_CLIENTE y FONO_CLIENTE dependen del cliente, no directamente
--     de la orden. ID_ORDEN → RUT_CLIENTE → {NOM_CLIENTE, FONO_CLIENTE})
--
--   → ID_ORDEN → PATENTE_AUTO → MARCA_AUTO
--     (MARCA_AUTO depende del vehículo, no de la orden.
--     ID_ORDEN → PATENTE_AUTO → MARCA_AUTO)
--
--   → ID_ORDEN → MECANICO_RUT → MECANICO_NOM
--     (MECANICO_NOM depende del mecánico, no directamente de la orden.
--     ID_ORDEN → MECANICO_RUT → MECANICO_NOM)
--
--   ANOMALÍAS: Si un cliente cambia su teléfono, habría que actualizar todas
